...
class MainSequentialFlow(SequentialTaskCollection):
    def __init__(self, **kwargs):
        ...
        if config["file_preparation"]["activated"] == 'True':
            self.initial_tasks = [FilePreparation(name = "file_preparation", **kwargs)]
        else:
    ...
    def next(self, done):
        ...
        if last_task != self.tasks[-1]:
            ...
            # SequencewiseParallelFlow task needs to be initialized using the output from previous steps.
