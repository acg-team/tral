...
from tral.repeat import repeat, repeat_io
...
def create_test_set(n=3,
    ...
    tests = [{'n': n,
              ...
              'flank_length': flank_length_multiplier * iL,
              'divergence': iD,
              'seed': {},
              'detectable': {}} for iL,
    ...
    for iT in tests:
        ...
        if iT['divergence'] > 0:
            tandem_repeats = list(
                repeat_io.evolved_tandem_repeats(
                    l=iT['l'],
                    n=2 * iT['n'],
                    n_samples=iT['n_samples'],
                    mutationRate=iT['divergence'],
            ...
            while len(tandem_repeats) < iT['n_samples']:
                ...
                tandem_repeats.extend(
                    [[iTR] * (2 * iT['n']) for iTR in new_repeats if not 'X' in iTR])
        ...
        # Create complete test (seed and detectable) sequences from the first
        # and second half, respectively, of the random flanks set
        iT['seed']['sequences'] = [
            iF[
                :iT['flank_length']] +
            "".join(
                iTR.msa) +
            iF[
                iT['flank_length']:] for iF,
            iTR in zip(
                random_flanks[
                    :iT['n_samples']],
                iT['seed']['tandem_repeats'])]
        iT['detectable']['sequences'] = [
            iF[
                :iT['flank_length']] +
            "".join(
                iTR.msa) +
...
def run_hmms_on_test_set(tests, divergence, parameter_ID):
    ...
    for iT in tests:
        ...
        for iS in range(iT['n_samples']):
            ...
            # Copy the amino acid sequence for each leaf into the
            # sequence.Sequence object <my_sequence>.
            my_sequence.sequence = iT['detectable']['sequences'][iS]

            # Create a viterbi.Viterbi object from the search sequence
            # <my_sequence> and the HMM <my_HMM>.
            my_viterbi = viterbi.Viterbi(iHMM, my_sequence.sequence)

            # Run the Viterbi algorithm.
            my_most_likely_path = my_viterbi.viterbi()

            # Convert the Viterbi path into a tandem repeat. Calculate the
            # shift towards the original tandem repeat
            # (my_tandem_repeat.shift).
            ...
            if my_tandem_repeat is None:
                iT[parameter_ID]['left_greediness'].append(None)
                iT[parameter_ID]['right_greediness'].append(None)
                print('no!')
            else:
                iT[parameter_ID]['left_greediness'].append(
                    iT['flank_length'] -
                    my_tandem_repeat.begin)
                iT[parameter_ID]['right_greediness'].append(
                    my_tandem_repeat.begin +
                    my_tandem_repeat.sequence_length -
                    iT['flank_length'] -
                    iT['n'] *
                    iT['l'])
