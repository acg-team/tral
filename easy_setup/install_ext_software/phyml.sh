...
if [ ! -d $TRAL_EXT_SOFTWARE/PhyML-3.1 ]; then # test if not already in directory
    ...
    rm -rf $TRAL_EXT_SOFTWAREPhyML-3.1.zip
fi


# Create an executable file PhyML

cp $TRAL_EXT_SOFTWARE/PhyML-3.1/PhyML-3.1_linux64 /usr/local/bin/  # copy wrapper file to execute PhyML into system path
chmod +x /usr/local/bin/PhyML-3.1_linux64 && echo -e "\nPhyML is in your system path /usr/local/bin/ and can be executed with the command \"PhyML-3.1_linux64\""
